...
/* <div class="question"><span>4</span>There is a preconfigured variable called 'pokemonList'. As the user
            ...
            for (var i = 0; i < questions.length; i++) {
                if (question.innerHTML.contains("")) {
                    questions[i].style.display = "none";
                } else {
                    questions[i].style.display = "";
                }
            }
            question.innerHTML = "<span>4</span>There is a preconfigured variable called 'pokemonList'. As the user types, check if the user already exists in that array. If yes, change the alert text to \"Username already exists\". If not, change the alert to \"Unique username created\"";
            var labelGrid = document.createElement("div");
            labelGrid.setAttribute("class", "label-grid");
